...
import { Direction } from '../src/coordinates';
...
describe('RectangularGrid Border Functionality', () => {
  describe('Border Wall Management', () => {
    ...
    test('should identify border directions correctly', () => {
      expect(grid.isBorderDirection({ row: 0, col: 0 }, Direction.North)).toBe(true);
      expect(grid.isBorderDirection({ row: 0, col: 0 }, Direction.West)).toBe(true);
      expect(grid.isBorderDirection({ row: 0, col: 0 }, Direction.South)).toBe(false);
      expect(grid.isBorderDirection({ row: 0, col: 0 }, Direction.East)).toBe(false);
      
      expect(grid.isBorderDirection({ row: 1, col: 1 }, Direction.North)).toBe(false);
      expect(grid.isBorderDirection({ row: 1, col: 1 }, Direction.South)).toBe(false);
    });
